...
* Centralized media manager integration: a first-class product primary image independent of the additional-image gallery; variant effective image inheritance; gallery ordering.
...
* Persistent working drafts and autosave state for **editing existing products** (never for Add New Product).
* A bin for discarded drafts and archived products (restore or permanently delete).
...
* **Three-Level Property Inheritance:** Manual variant override > Attribute-level override > Product default. This applies to price, cost, images, and weight. **Preorder is product-only** — variants have no preorder field; if the product has preorder on, any variant can be pre-ordered. Inherited fields always show the **actual inherited value** in the control (text, image, select — never the word "inherited" as a value). After the user changes a field it is no longer inherited; show that clearly with an Inherit action that restores the product/attribute value.
...
* **Product Creation/Editing Flow:** Create and edit share the same editor. Section chips, in this order:
  ...
  4. Attributes & Variations (select attributes and values; generate combinations; filters; bulk edit of selected variants and filter-targeted variants with dynamic fields). **Add New Product and Edit Product** use inline row-based variant inputs (no dialog). **Product list and the view-only product page** use compact rows; editing a saved variant opens a dialog. Image assignment is not part of attribute-value selection.
  5. Product Description (its own section).
  6. SEO (last).
  Pricing (current price, % or flat discount, server-calculated sell price, tax rate, **packaging cost**) renders after Information and is not a section chip. Packaging cost lives only in Pricing — never in SEO/Publication.
* **Images:** Primary image is stored on `Product.primaryImageMediaId` and is independent of `ProductImage`. Additional images never include the primary. Do not assign images while selecting attribute values.
...
* **Drafts and bin:** Add New Product **never** resumes or autosaves a persistent draft — every New Product click is a fresh session, with no Resume-draft UI. Existing-product edit: one active draft per `productId`; autosave after real changes only (debounce, skip identical/reverted); autosave ≠ publish; resume/discard on edit; isolate by `productId`; no create↔edit contamination. Drafts belong on the product list (unsaved-draft badge) and the edit page, not on create. Do not create duplicate drafts. Discarding an unpublished DRAFT bins the product. The bin supports restore and permanent delete through in-app confirmation dialogs, never `alert()`/`confirm()`.
* **Navigation:** The admin sidebar **slides open and closed** as a whole and is **open by default**. **Products is a collapsible parent** with indented children (Add new, Product list, Categories, Attributes, Brands, Price lists, Tax rates, Packaging costs, Unit labels). Clicking the parent expands/collapses and does **not** navigate. Auto-expand when the current route is a Products child. The active submenu item is the longest matching href (so Add new wins over Product list on **global** Platform item, not a product-only item. Inventory stays under Stock. There is no Labels nav item and no Label field on the product form.
...
1. Simple products (one variant, no attribute matrix).
2. Variable products with multiple variants generated from selected attribute values.
...
* Each variant has a stable ID and a deterministic option key. Variants do **not** have a SKU.
...
* Variants may inherit product-level pricing, cost, and weight. Preorder is **not** a variant property.
...
* **Add New Product and Edit Product:** variant fields are **inline row-based inputs** (image, current price, discount type, discount, sell price, weight). No edit dialog. **Product list and the view-only product page:** compact rows; editing a saved variant opens a dialog. Do not mix the two workflows.
...
* Product list (no inventory columns).
* Search.
* Filtering.
* Sorting.
* Pagination.
* Create (`/admin/catalog/products/new`) and edit (`/admin/catalog/products/:id/edit`) using the same form.
* View-only product page (`/admin/catalog/products/:id`).
* Variant generation inside the editor (no separate variant page).
...
* Edit-page draft autosave and recovery (existing products only), with a leave confirmation dialog. Add New Product has no resume/autosave draft.
* Archive / bin / restore / permanent delete with custom in-app dialogs.

Do not allow deleting products in ways that break order history.

Do not show inventory on the product list. Inventory is a separate module.

Do not mutate stock when creating a product.
...
Preorder is a **product-only** setting. Variants have no preorder field. When the product has preorder on, any variant of that product can be pre-ordered.
...
* Variable products.
* Variant generation.
* Product-level SKU uniqueness (no variant SKU).
