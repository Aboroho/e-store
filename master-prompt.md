...
* **Product Creation/Editing Flow:** Create and edit share the same editor. Section chips, in this order:
  ...
  4. Attributes & Variations (select attributes and values; generate combinations; filters; bulk edit of selected variants and filter-targeted variants with dynamic fields). **Add New Product** uses inline row-based variant inputs (no dialog). **Product list and single product view/edit** use compact rows; editing a saved variant opens a dialog. Image assignment is not part of attribute-value selection.
...
* **Navigation:** The admin sidebar **slides open and closed** as a whole and is **open by default**. **Products is a collapsible parent** with indented children (Add new, Product list, Categories, Attributes, Brands, Price lists, Tax rates, Packaging costs, Unit labels). Clicking the parent expands/collapses and does **not** navigate. Auto-expand when the current route is a Products child. The active submenu item is the longest matching href (so Add new wins over Product list on `/new`). Bin is a **global** Platform item, not a product-only item. Inventory stays under Stock. There is no Labels nav item and no Label field on the product form.
...
* **Add New Product:** variant fields are **inline row-based inputs** (image, current price, discount type, discount, sell price, weight). No edit dialog. **Product list and single product view/edit:** compact rows; editing a variant opens a dialog. Do not mix the two workflows.
