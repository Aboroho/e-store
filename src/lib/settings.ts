...
export const BUSINESS_SETTINGS = {
  ...
  orderMaxDiscountPercent: define({ key: "order.max_discount_percent", label: "Maximum order discount (%)", description: "Upper limit for an order-level discount, as a percentage of the net item value. Item-level discounts are always limited to their own line.", group: "Orders", defaultValue: 100 }),
  orderListDefaultColumns: define({ key: "order.list_default_columns", label: "Default order list columns", description: "Columns every user sees in the order list unless they saved their own preference. Users can only pick columns they are allowed to see.", group: "Orders", defaultValue: ["order", "customer", "phone", "type", "status", "collectible", "creator", "created"], valueType: "JSON" }),
  ...
  mediaMaxUploadBytes: define({ key: "media.max_upload_bytes", label: "Maximum upload size (bytes)", description: "Largest single media upload accepted by the media manager.", group: "Media", defaultValue: 15 * 1024 * 1024 }),
  mediaAllowedTypes: define({ key: "media.allowed_types", label: "Allowed file types", description: "MIME types accepted by the media manager.", group: "Media", defaultValue: ["image/jpeg", "image/png", "image/webp", "image/avif", "image/gif", "application/pdf"], valueType: "JSON" }),
  reviewRequirePurchase: define({ key: "review.require_purchase", label: "Require a purchase", description: "Only customers with a delivered order may review a product.", group: "Reviews", defaultValue: true }),
  reviewModerationRequired: define({ key: "review.moderation_required", label: "Moderate reviews", description: "Reviews stay pending until an administrator approves them.", group: "Reviews", defaultValue: true }),
  customerAutoApproveReview: define({ key: "reviews.auto_publish_verified", label: "Publish verified reviews immediately", description: "Verified purchase reviews are published without waiting for moderation.", group: "Reviews", defaultValue: false }),
